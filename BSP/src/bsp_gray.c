...
#define GRAY_PORT_A GRAY_GPIOA_IN_PORT
#define GRAY_PORT_B GRAY_GPIOB_IN_PORT
...
static uint8_t _ReadRaw(void)
{
    uint32_t pinsA = DL_GPIO_readPins(GRAY_PORT_A, GRAY_OUT5);
    uint32_t pinsB = DL_GPIO_readPins(GRAY_PORT_B, GRAY_PORT_B_MASK);
    uint8_t value = 0U;

    if ((pinsB & GRAY_OUT1) != 0U) { value |= 0x80U; }
    if ((pinsB & GRAY_OUT2) != 0U) { value |= 0x40U; }
    if ((pinsB & GRAY_OUT3) != 0U) { value |= 0x20U; }
    if ((pinsB & GRAY_OUT4) != 0U) { value |= 0x10U; }
    if ((pinsA & GRAY_OUT5) != 0U) { value |= 0x08U; }
    if ((pinsB & GRAY_OUT6) != 0U) { value |= 0x04U; }
    if ((pinsB & GRAY_OUT7) != 0U) { value |= 0x02U; }
    if ((pinsB & GRAY_OUT8) != 0U) { value |= 0x01U; }
